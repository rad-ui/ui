...
import { SSRDialog } from './ssr-components';
import { bundleForSSR } from './utils';

test('Dialog SSR hydrates without warnings', async ({ page }) => {
  const html = ReactDOMServer.renderToString(React.createElement(SSRDialog));
