...
import { SSRTooltip } from './ssr-components';
import { bundleForSSR } from './utils';

test('Tooltip SSR hydrates without warnings', async ({ page }) => {
  const html = ReactDOMServer.renderToString(React.createElement(SSRTooltip));
